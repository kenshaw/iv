...
// The expectations here are written against measure rather than against
// particular strings: what fits depends on the font that happens to be
// installed, so anything else would only hold on the machine it was written
// on.

func TestFit(t *testing.T) {
	const size = 10
	s := "a string long enough that it has to be cut somewhere"
	if got := fit(s, size, false, int(measure(s, size, false))+10); got != s {
		t.Errorf("expected a string that fits to be kept, got %q", got)
	}
	if got := fit("", size, false, 100); got != "" {
		t.Errorf("expected the empty string, got %q", got)
	}
	const width = 60
	switch got := fit(s, size, false, width); {
	case got == s:
		t.Error("expected a truncation")
	case !strings.HasSuffix(got, "…"):
		t.Errorf("expected an ellipsis, got %q", got)
	default:
		if w := measure(got, size, false); w > width {
			t.Errorf("expected %q to fit in %d, it measures %v", got, width, w)
		}
	}
	if got := fit(s, size, false, 1); got != "" {
		t.Errorf("expected nothing to fit in 1 unit, got %q", got)
	}
}
...
func TestShrink(t *testing.T) {
	sizes := []int{40, 20}
	const s = "abcde"
	if size, got := shrink(s, sizes, true, int(measure(s, 40, true))+10); size != 40 || got != s {
		t.Errorf("expected the largest size to be kept, got %d %q", size, got)
	}
	if size, got := shrink(s, sizes, true, int(measure(s, 20, true))+10); size != 20 || got != s {
		t.Errorf("expected a step down rather than a truncation, got %d %q", size, got)
	}
	// too narrow for the whole string even at the smallest size, but wide
	// enough for a truncation of it
	narrow := int(measure("ab…", 20, true)) + 1
	if size, got := shrink(s, sizes, true, narrow); size != 20 || !strings.HasSuffix(got, "…") {
		t.Errorf("expected a truncation at the smallest size, got %d %q", size, got)
	}
	// and narrower than a single character plus the ellipsis
	if size, got := shrink(s, sizes, true, 1); size != 20 || got != "" {
		t.Errorf("expected nothing to fit, got %d %q", size, got)
	}
}

func TestMeasure(t *testing.T) {
	// whether or not a font was found, these have to hold for fit to work
	if measure("ii", 20, false) <= measure("i", 20, false) {
		t.Error("expected a longer string to measure wider")
	}
	if measure("abc", 40, false) <= measure("abc", 20, false) {
		t.Error("expected a larger size to measure wider")
	}
	if measure("", 20, false) != 0 {
		t.Error("expected the empty string to measure zero")
	}
}

func TestFontFamily(t *testing.T) {
	if !strings.HasSuffix(fontFamily(), "sans-serif") {
		t.Errorf("expected the generic family to come last, got %q", fontFamily())
	}
}
